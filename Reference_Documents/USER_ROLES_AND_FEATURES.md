...
| Shown to | Org portal, Student portal (not SuperAdmin layout) |
|----------|-----------------------------------------------------|
...
| 12 | Revenue & Payments | `/admin/revenue` | ⚠️ Nav link only — **no route** |
...
**Read-only oversight** — no approve/reject/edit/delete in SuperAdmin UI.
...
> **UI note:** View toggle button labels are **swapped** — button labeled "Overview" shows the **card grid**; button labeled "Manage Plans" shows the **filterable table**.
...
### 4.12 Create Notification — `/admin/create-notification`
...
### 4.13 System Health — `/admin/health`
...
*Profile via header. Announcement banner visible in org layout.*
...
| Subscribe | Modal: dates, exam count, auto-renew checkbox; immediate activation (no payment gateway yet) |
...
| Subscribe | Immediate activation (no payment gateway yet) |
...
| Item | Detail |
|------|--------|
| `/admin/revenue` | Sidebar link exists; **no route or page** — revenue visible on Dashboard + Subscriptions |
| Subscription Plans tab labels | "Overview" / "Manage Plans" buttons are **swapped** vs content |
| SuperAdmin Question Bank | View-only; moderation is via Reviewer portal |
| SuperAdmin Subscriptions | View-only; no cancel/refund UI |
| Bulk MCQ file import | Documented in [BULK_QUESTION_UPLOAD.md](./BULK_QUESTION_UPLOAD.md); **not implemented** |
| Payment gateway | Subscribe flows activate immediately without payment processing |
| `/org/*` route guard | Frontend allows any org-authenticated role to hit URLs; **API enforces OrgAdmin** on sensitive writes |
| Announcement banner | Not shown in SuperAdmin layout |
| CI smoke placeholders | GitHub Actions uses dummy Supabase/JWT values — not real secrets |

---

*This document is derived from the implemented UI routes (`frontend/src/features/**/routes.jsx`), page components, and backend route modules. Update when features ship or nav changes.*
