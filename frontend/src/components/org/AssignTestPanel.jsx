...
export default function AssignTestModal({ test, onClose, onSuccess }) {
  ...
  const [eligibleMeta, setEligibleMeta] = useState({ activeStudents: 0, eligibleStudents: 0 });
  ...
  const loadData = async () => {
    try {
      ...
      const [studentsRes, groupsRes] = await Promise.allSettled([
        testAPI.getEligibleStudents(getTestId(test), { limit: 1000 }),
        groupAPI.getGroups({ limit: 1000 }),
      ]);

      if (studentsRes.status === 'fulfilled') {
        const rows = (studentsRes.value.students || []).map((s) => ({
        ...
        setStudents(rows);
        setEligibleMeta({
          activeStudents: Number(studentsRes.value?.totals?.activeStudents || 0),
          eligibleStudents: Number(studentsRes.value?.totals?.eligibleStudents || rows.length),
        });
      } else {
        setEligibleMeta({ activeStudents: 0, eligibleStudents: 0 });
      }
        ...
        <form className="modal-form" onSubmit={handleSubmit}>
          ...
          {assignmentType === 'all' && (
            <div className="notice info">
              <AlertCircle size={18} />
              <span>This will assign the test to all exam-enrolled students in your organization.</span>
            </div>
          )}
          ...
          <div className="notice info" style={{ marginTop: 4 }}>
            <AlertCircle size={18} />
            <span>
              Eligible students for this exam: {eligibleMeta.eligibleStudents}
              {eligibleMeta.activeStudents > 0 ? ` / ${eligibleMeta.activeStudents} active students` : ''}
            </span>
          </div>
...
export function AssignTestPanelEmbedded({ test, onAssigned, canonicalTestId }) {
  ...
  const loadData = async () => {
    try {
      setLoadingData(true);
      const [studentsRes, groupsRes] = await Promise.allSettled([
        testAPI.getEligibleStudents(tid, { limit: 1000 }),
        groupAPI.getGroups({ limit: 1000 }),
      ]);
      if (studentsRes.status === 'fulfilled') {
        const rows = (studentsRes.value.students || []).map((s) => ({
    ...
    <div className="tw-assign-embedded">
      ...
      <form className="modal-form" onSubmit={handleSubmit}>
        ...
        {assignmentType === 'all' && (
          <div className="notice info">
            <AlertCircle size={18} />
            <span>Assigns to all exam-enrolled students in your organization.</span>
          </div>
        )}
