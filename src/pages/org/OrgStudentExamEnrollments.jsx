...
export default function OrgStudentExamEnrollments() {
  ...
  }, [dirPage, dirSearch, dirStatus, dirExamId]);

  useEffect(() => {
    setDirPage(1);
  }, [dirSearch, dirStatus, dirExamId]);
            ...
            <div className="qb-filters-grid">
              ...
              <div className="qb-filter-field">
                ...
                >
                  ...
                  <option value="Implicit">Implicit (subscription only)</option>
                      ...
                      {directoryRows.length === 0 ? (
                        ...
                        directoryVisibleRows.map((r) => (
                          <tr key={r.enrollmentId || `imp-${r.studentId}-${r.examId}`}>
                            ...
                            <td className="cell-date">{fmtTs(r.reviewedAt)}</td>
                            <td className="cell-er-wrap">{r.reviewNote || '—'}</td>
                            <td className="cell-date">{fmtTs(r.approvedAt)}</td>
                            <td className="cell-date">{fmtTs(r.withdrawnAt)}</td>
                            <td>{r.withdrawalInitiatedBy || '—'}</td>
                            <td className="cell-er-wrap">{r.withdrawalReason || '—'}</td>
                            <td className="cell-date">{fmtTs(r.createdAt)}</td>
                            <td className="cell-date">{fmtTs(r.updatedAt)}</td>
